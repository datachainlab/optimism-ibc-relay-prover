...
func run(ctx context.Context) error {
	...
	l1Header1, _, err := proverL1Client.GetFinalizedL1Header(ctx, latestMetadata.L1Head)
	if err != nil {
		return errors.WithStack(err)
	}
	l1Header2, _, err := proverL1Client.GetFinalizedL1Header(ctx, latestMetadata.L1Head)
	if err != nil {
		return errors.WithStack(err)
	}
